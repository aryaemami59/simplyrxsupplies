...
/**
 * Returns the previous value of a given state or prop. This hook stores the
 * most recent render's value in a ref and returns the value from the previous
 * render. On the initial render, it returns **`undefined`**.
 *
 ...
 *
 *   useEffect(() => {
 *     console.log("Count changed from %o to %o", previousCount, count);
 *   }, [count, previousCount]);
 *
 *   return (
 *     <button
...
export const usePreviousState =
   ...
   */
  <StateType>(currentState: StateType): StateType | undefined => {
