...
const SideBarAccordion = ({ category }) => {
    ...
    return (<div key={`div-SideBarAccordion-${category}-outer`}>
      <h2 key={`h2-SideBarAccordion-${category}`} className="accordion-header">
        <button key={`button-SideBarAccordion-${category}`} onClick={toggle} className={`accordion-button rounded ${darkTheme ? "custom-dark-mode" : "custom-light-mode"} ${open ? "" : COLLAPSED}`}>
          {category}
        </button>
      </h2>
