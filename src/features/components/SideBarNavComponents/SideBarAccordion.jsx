...
const SideBarAccordion = ({ category }) => {
    ...
    return (<div key={`div-SideBarAccordion-${category}-outer`}>
      <h2 key={`h2-SideBarAccordion-${category}`} className="accordion-header">
        <button key={`button-SideBarAccordion-${category}`} onClick={toggle} className={`accordion-button rounded ${darkTheme
            ? "custom-dark-mode custom-text-shadow-white"
            : "custom-light-mode custom-text-shadow-white-50"} ${open ? "" : COLLAPSED}`}>
          {category}
        </button>
      </h2>
          ...
          <Card key={`Card-SideBarAccordion-${category}`} className={`bg-gradient custom-dark-mode ${darkTheme ? "bg-dark" : "bg-light"}`}>
            <Card.Body key={`Card.Body-SideBarAccordion-${category}`} className={`bg-gradient custom-dark-mode ${darkTheme ? "bg-dark" : "bg-light"}`}>
              <ListGroup key={`ListGroup-SideBarAccordion-${category}`}>
                {sidebarItems.map((f) => (<SingleSideBarAccordionListItem category={category} itemObj={f} key={`${f.name}-SingleSideBarAccordionListItem`}/>))}
              </ListGroup>
            </Card.Body>
          </Card>
        </div>
