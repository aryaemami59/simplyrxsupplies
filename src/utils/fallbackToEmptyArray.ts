import { EMPTY_ARRAY } from "./emptyArray.js"
import { isEmptyArray } from "./predicates/isEmptyArray.js"

/**
 * This function is used to maintain referential integrity of a brand new array
 * that is usually the result of calling
 * {@linkcode Array.map} or {@linkcode Array.filter}.
 * It checks to see if the resulting array is empty and if it is,
 * instead of returning the brand new empty array,
 * it will return {@linkcode EMPTY_ARRAY}
 * in order to prevent unnecessary reassignment of state values and
 * maintain referential integrity as much as possible.
 *
 * @param array - The array whose length will be checked.
 * @returns Either an {@linkcode EMPTY_ARRAY | empty array} or the {@linkcode array | array parameter}.
 * @see {@linkcode EMPTY_ARRAY}
 */
export const fallbackToEmptyArray = <ArrayElement>(
  array: ArrayElement[] | undefined,
): ArrayElement[] => {
  if (array == null) {
    return EMPTY_ARRAY
  }

  if (Object.is(array, EMPTY_ARRAY)) {
    return array
  }

  return isEmptyArray(array) ? EMPTY_ARRAY : array
}
